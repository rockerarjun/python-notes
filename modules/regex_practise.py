...
catmatch = re.findall("\s[acboe]at\s",cat)#only three charatcer match and comes at ,at last# hw)
print ("catmatch:",catmatch)
#quit()
 # [a-zA-Z0-9] - range of possible characters
...
#meta = re.findall(r'[A-Za-z]{2}',sentences) # 2 chars
#meta = re.findall(r'[A-Za-z]{3}',sentences) 
#print("\n1[A-Za-z]{2} : ",meta)
#print("\n[A-Za-z]{3} : ",meta)


#meta = re.findall(r'\b[A-Za-z]{2}\s',sentences) #with spaces after 2 char
#print("\n2 [A-Za-z]{2}\s : ",meta)
#meta = re.findall(r'^(.*)\s(.*?).*',sentences) 
#print("\n^(.*)\s(.*?).* : ",meta)
#quit()
meta = re.findall(r'\s(.*)\, \s)',sentences) 
print("\n\s(.*)\, \s) : ",meta)
quit()



#meta = re.findall(r'[0-9]{2}',sentences) 
#print("\n2 [0-9]{2} : ",meta)
...
#meta = re.findall(r'([a-zA-Z]+)\s(\w+)',sentences) #\w+ [a-zA-Z_0-9]
#print("\n6 ([a-zA-Z]+)\s(\w+) : ",meta)
quit()
